...
def is_swap(player_score, opponent_score):
    ...
    return abs(playerones-playertens)==abs(opponentones-opponenttens)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    highestdice, highestvalue=0,0
    for x in range(10):
        value = make_averaged(roll_dice,num_samples)(x+1, dice)
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(free_bacon_score, opponent_score):
        if free_bacon_score<opponent_score:
            return 0
        if free_bacon(opponent_score)>margin and free_bacon_score<opponent_score:
            return 0
